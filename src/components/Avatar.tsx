...
const sizeForPosition = (position: number) => {
  switch (position) {
    case 1:
      return { cls: "h-9 w-9 border-yellow-500 dark:border-yellow-400", px: 36 };
    case 2:
      return { cls: "h-8 w-8 border-neutral-500 dark:border-neutral-400", px: 32 };
    case 3:
      return { cls: "h-7 w-7 border-orange-700 dark:border-orange-600", px: 28 };
    default:
      return { cls: "h-6 w-6 border-white dark:border-gray-900", px: 24 };
  }
};
    ...
    <div className={classNames("flex -space-x-1 overflow-hidden", className)}>
      {avatars.map((a) => {
        ...
        const { cls, px } = rankedSizing && a.position
          ...
          : { cls: "h-8 w-8 border-white dark:border-gray-900", px: 32 };
